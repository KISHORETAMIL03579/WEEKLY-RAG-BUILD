...
class TestPolicyRouter:
    ...
    @pytest.mark.parametrize("question,emp", [
        ("What is EMP004's resignation notice?", "EMP004"),
        ("How much sick leave accrual rate does EMP006 get?", "EMP006"),
        ("Is EMP009 eligible to receive the 10% pension contribution allowance?", "EMP009"),
        ("How many unused annual leave days can EMP002 carry forward?", "EMP002"),
        ("What severance pay is EMP008 entitled to upon termination for unsatisfactory performance?", "EMP008"),
        ("How many days of accrued unused annual leave can EMP010 commute to cash?", "EMP010"),
    ])
    def test_simple_queries_route_to_workflow(self, question, emp):
        d = route_policy_question(question, emp)
        assert d.mode == MODE_WORKFLOW, (
    ...
    def test_router_returns_reason(self):
        d = route_policy_question("What is EMP001's annual leave entitlement?", "EMP001")
    ...
    def test_router_returns_complexity(self):
        d = route_policy_question("What is EMP001's annual leave entitlement?", "EMP001")
        assert d.complexity in (COMPLEXITY_SIMPLE, COMPLEXITY_MODERATE, COMPLEXITY_COMPLEX)
    ...
    def test_routing_id_is_unique(self):
        d1 = route_policy_question("What is EMP001's annual leave entitlement?", "EMP001")
        d2 = route_policy_question("What is EMP002's annual leave entitlement?", "EMP002")
        assert d1.routing_id != d2.routing_id, "Each routing decision must have a unique ID"
...
class TestPolicySchemaConstants:
    ...
    def test_token_cost_per_1m_is_single_source(self):
        ...
        assert TOKEN_COST_PER_1M == 0.50, f"TOKEN_COST_PER_1M must be 0.50, got {TOKEN_COST_PER_1M}"
        expected_rate = TOKEN_COST_PER_1M / 1_000_000
        assert abs(TOKEN_COST_PROXY_RATE - expected_rate) < 1e-12, (
...
class TestRetryLogic:
    ...
    def test_non_retryable_budget_reason(self):
        ...
        for reason in ("BUDGET_ITERATIONS", "BUDGET_TOKENS", "BUDGET_COST", "BUDGET_WALL_CLOCK"):
            assert not _is_retryable(reason), f"{reason} should NOT be retryable"

    def test_retryable_transient_reasons(self):
        from backend.routes.policy import _is_retryable
        for reason in ("OLLAMA_TIMEOUT", "TRANSIENT_NETWORK", "MODEL_ERROR", "TOOL_ERROR"):
    ...
    def test_retry_accumulation_math(self):
        ...
        assert abs(total_cost - 1400 * TOKEN_COST_PROXY_RATE) < 1e-10, "Cost must use accumulated tokens"
    ...
    def test_cost_formula_uses_token_cost_per_1m(self):
        ...
        assert abs(expected_cost - computed_cost) < 1e-12, (
...
class TestBenchmarkCaseIsolation:
    ...
    def test_week6_evaluation_type_is_separate(self):
        ...
        assert week7_type != week6_type, "Week 6 and Week 7 must have distinct evaluation types"
    ...
    def test_week6_frozen_benchmark_intact(self):
        ...
        if not cases_file.exists():
            pytest.skip("Week 6 eval_cases_25.json not found — skipping integrity check")
        with open(cases_file, "r") as f:
            cases = json.load(f)
        assert len(cases) == 25, f"Week 6 frozen benchmark must have 25 cases, found {len(cases)}"
    ...
    def test_p50_is_real_measurement(self):
        ...
        latencies = [19401.5, 21188.2, 21173.2, 20490.1, 20917.8, 21009.9, 8971.3, 8905.3, 8915.1, 9127.1]
...
class TestPolicyAPIRoutes:
    ...
    def test_classify_endpoint_simple_question(self):
        ...
        response = client.get(
            ...
            params={"question": "What is EMP001's annual leave entitlement?", "employee_id": "EMP001"}
        )
    ...
    def test_search_endpoint_returns_latency(self):
        ...
        response = client.post(
            "/api/policy/search",
            json={"employee_id": "EMP001", "question": "What is EMP001's annual leave entitlement?"}
        )
        assert response.status_code == 200
        data = response.json()
