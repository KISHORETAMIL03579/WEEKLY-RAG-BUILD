...
class TestWeek6Evaluation(unittest.TestCase):
    ...
    def test_eval_cases_dataset_integrity(self):
        ...
        self.assertGreaterEqual(
            len(cases), 25, "Must have at least 25 evaluation cases"
        )
        ...
        for case in cases:
            for k in required_keys:
                self.assertIn(
                    k, case, f"Case {case.get('case_id')} missing required key '{k}'"
                )
            self.assertTrue(
                bool(case["question"].strip()), "Question must not be empty"
            )
    ...
    def test_taxonomy_mode_coverage(self):
        ...
        for case in cases:
            ...
            self.assertIn(
                mode,
                WEEK5_TAXONOMY_MODES,
                f"Invalid taxonomy mode '{mode}' in case {case['case_id']}",
            )
            modes_present.add(mode)

        self.assertEqual(
            len(modes_present), 5, "All 5 Week-5 taxonomy modes must be represented"
        )
    ...
    def test_regression_cases_present_and_bound(self):
        ...
        self.assertGreaterEqual(
            regression_count, 2, "Must contain at least 2 verbatim regression cases"
        )
    ...
    def test_blind_human_labels_integrity(self):
        ...
        self.assertEqual(
            len(labels), len(cases), "Labels count must match evaluation cases count"
        )
        for case in cases:
            cid = case["case_id"]
    ...
    def test_deterministic_assertion_section_present(self):
        ...
        self.assertTrue(
            policy_section_reference_present("According to section 5.3.2 of the policy")
        )
        self.assertTrue(policy_section_reference_present("As stated in Section: 2.2.2"))
        self.assertTrue(
            policy_section_reference_present(
                "Sec 9.1.1 provides the grievance timeline"
            )
        )
        self.assertFalse(
            policy_section_reference_present("Employees receive 28 days of leave [1].")
        )
    ...
    def test_deterministic_assertion_section_resolves(self):
        ...
        self.assertTrue(
            policy_section_reference_resolves("As per section 2.2.3 and section 9.1")
        )
        self.assertTrue(
            policy_section_reference_resolves(
                "Referencing Section 4.3.1 Salary Advances"
            )
        )
        self.assertTrue(
            policy_section_reference_resolves("Section 10.5.3 applies to ill health")
        )
        self.assertTrue(
            policy_section_reference_resolves("Section 5.3.2.1 sub-tier entitlement")
        )
        self.assertFalse(
            policy_section_reference_resolves("According to section 99.88.77")
        )
        self.assertFalse(
            policy_section_reference_resolves("Section 42.1 is completely fabricated")
        )
        self.assertTrue(
            policy_section_reference_resolves(
                "Employees receive leave without section citations."
            )
        )
    ...
    def test_eval_cases_failure_categorization_schema(self):
        ...
        for case in cases:
            self.assertIn(
                "failure_category",
                case,
                f"Case {case.get('case_id')} missing 'failure_category'",
            )
            self.assertIn(
                "failure_type",
                case,
                f"Case {case.get('case_id')} missing 'failure_type'",
            )
            self.assertIn(
                "failure_reason",
                case,
                f"Case {case.get('case_id')} missing 'failure_reason'",
            )
            self.assertIn(
                "resolution", case, f"Case {case.get('case_id')} missing 'resolution'"
            )
    ...
    def test_deterministic_assertion_numeric_value(self):
        ...
        self.assertTrue(
            numeric_policy_value_present(
                "Entitlement is sixteen (16) weeks", "sixteen (16) weeks"
            )
        )
        self.assertTrue(
            numeric_policy_value_present("You get 5 days of leave", "5 days")
        )
        self.assertTrue(
            numeric_policy_value_present(
                "Working hours are 9.00 am to 5:30 pm", "9.00 am to 5:30 pm"
            )
        )
        self.assertTrue(numeric_policy_value_present("No number needed", None))
        self.assertFalse(
            numeric_policy_value_present("You get several days of leave", "5 days")
        )
    ...
    def test_deterministic_assertion_refusal_behavior(self):
        ...
        self.assertTrue(
            out_of_jurisdiction_refusal("I don't know.", is_out_of_jurisdiction=True)
        )
        self.assertTrue(
            out_of_jurisdiction_refusal(
                "The documents do not mention overtime.", is_out_of_jurisdiction=True
            )
        )
        self.assertFalse(
            out_of_jurisdiction_refusal(
    ...
    def test_assertion_counts_constant(self):
        ...
        self.assertEqual(
            DETERMINISTIC_ASSERTION_COUNT, 5, "Must have 5 deterministic assertions"
        )
