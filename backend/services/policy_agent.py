...
from backend.config import OLLAMA_CHAT_MODEL, OLLAMA_URL, logger
...
DEFAULT_AGENT_MODEL = OLLAMA_CHAT_MODEL
...
def _tool_call_from_message(
    ...
    if not isinstance(calls, list) or len(calls) > 1:
        raise PolicyAgentError(
            "MODEL_ERROR", "Expected at most one tool call per ReAct step"
        )
    if not calls:
        return None
    call = calls[0]
...
def _failure_result(
    ...
    return PolicyOutputContract(
        ...
        token_source=(
            "ollama_live"
            if any(call.get("token_source") == "ollama_live" for call in llm_calls)
            else "unavailable"
        ),
        llm_calls=llm_calls,
...
def run_agent_case(
    ...
    if use_live_llm is False or (use_live_llm is None and not check_ollama_available()):
        return _failure_result(
            case_id,
            employee_id,
            question,
            "OLLAMA_UNAVAILABLE",
            "The Ollama model provider is unavailable; no answer was generated.",
            started,
            calls,
            llm_calls,
            prompt_tokens,
    ...
    system_prompt = (
        "You are a grounded HR policy ReAct agent. Choose at most one registered tool "
        "per step using native tool calls. Look up the requested employee before giving "
        "an employee-specific answer; never infer missing employee details. Use only "
        "observations returned by tools and the policy handbook for facts. When ready, "
        'return exactly one JSON object with string keys "entitlement_value", '
        '"rule_cited", and "explanation". Do not output any other fields.'
    )
    messages: List[Dict[str, Any]] = [
    ...
    while iteration < max_iterations:
        ...
        try:
            response, p_tokens, c_tokens, latency_ms = _call_ollama_step(
                messages,
                model=model,
                temperature=temperature,
                timeout=remaining,
            ...
            if exc.reason == "OLLAMA_TIMEOUT":
                reason = "BUDGET_WALL_CLOCK"
                message = "Ollama timed out before generating an answer."
                ...
                reason = exc.reason
                message = "The Ollama provider is temporarily unavailable."
            else:
                reason = exc.reason
        ...
        try:
            selected_tool = _tool_call_from_message(message)
        except PolicyAgentError as exc:
            return _failure_result(
                case_id,
                employee_id,
        ...
        if tool_name == "get_employee_record":
            if arguments["employee_id"].strip().upper() != employee_id.strip().upper():
                return _failure_result(
                    case_id,
                    employee_id,
                    question,
