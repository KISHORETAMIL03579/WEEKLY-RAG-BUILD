...
export const EvaluationDatasetManager: React.FC<
  EvaluationDatasetManagerProps
> = ({
  evaluatorType,
  title,
  builtinCount,
  ...
  const [parseResult, setParseResult] = useState<DatasetParseResult | null>(
    null,
  );
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.warnings && res.warnings.length > 0) {
        onNotify(
          `Dataset parsed with ${res.warnings.length} notice(s)`,
          "info",
        );
      }
    } catch (err: any) {
  ...
  const handleConfirmImport = () => {
    ...
    onNotify(
      `Successfully imported ${imported.length} valid cases into custom dataset`,
      "success",
    );
  };
  ...
  const handleSaveEdit = (e: React.FormEvent) => {
    ...
    if (isNewCase) {
      ...
      const safeId = exists
        ? `${editingCase.case_id}_${customCases.length + 1}`
        : editingCase.case_id;
      updated = [...customCases, { ...editingCase, case_id: safeId }];
    } else {
      updated = customCases.map((c) =>
        c.case_id === editingCase.case_id ? editingCase : c,
      );
    }
    ...
    onNotify(
      isNewCase
        ? `Added case ${editingCase.case_id}`
        : `Updated case ${editingCase.case_id}`,
      "success",
    );
  };
  ...
  const handleClearCustom = () => {
    ...
    onResetToBuiltin();
    onNotify(
      "Cleared custom dataset. Switched back to canonical benchmark.",
      "info",
    );
  };
  ...
  return (
    ...
    >
          ...
          <div>
            ...
            <div style={{ fontSize: "0.78rem", color: "var(--text-muted)" }}>
              ...
              <strong style={{ color: "#60a5fa" }}>
                {datasetMode === "custom"
                  ? `Custom Dataset (${customCases.length} cases)`
                  : `${builtinLabel} (${builtinCount} cases)`}
              </strong>
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            <button
              ...
              style={{
                ...
                borderRadius: "4px",
                background:
                  datasetMode === "builtin" ? "var(--accent)" : "transparent",
            ...
            <button
              ...
              onClick={() => {
                if (customCases.length === 0) {
                  onNotify(
                    'Custom dataset is empty. Click "Import Q&A File" or "+ Add Case" to add custom test cases.',
                    "info",
                  );
                  return;
                }
              ...
              style={{
                ...
                borderRadius: "4px",
                background:
                  datasetMode === "custom" && customCases.length > 0
                    ? "var(--accent)"
                    : "transparent",
                color:
                  datasetMode === "custom" && customCases.length > 0
              ...
              title={
                customCases.length === 0
                  ? "No custom cases yet. Import or add cases first."
          ...
          {customCases.length > 0 && (
            ...
            >
              {showCasesDrawer
                ? "Hide Custom Cases ▲"
                : `Manage Custom Cases (${customCases.length}) ▼`}
            </button>
          )}
      ...
      {isRunning && (
        ...
        >
          <span>🔒</span> Benchmark execution active. Dataset modifications are
          locked until execution finishes.
        </div>
      )}
        ...
        >
          ...
          >
            ...
            <span style={{ fontSize: "0.72rem", color: "var(--text-muted)" }}>
              Zero-Leakage Protection: Expected answers are isolated from LLM
              context
            </span>
          </div>
          ...
          <div style={{ maxHeight: "280px", overflowY: "auto" }}>
            ...
            >
              <thead>
                ...
                >
                  <th style={{ padding: "8px 12px", width: "100px" }}>
                    Case ID
                  </th>
                  {evaluatorType === "policy" && (
                    <th style={{ padding: "8px 12px", width: "90px" }}>
                      Emp ID
                    </th>
                  )}
                  <th style={{ padding: "8px 12px" }}>Question</th>
                  <th style={{ padding: "8px 12px", width: "120px" }}>
                    Ground Truth
                  </th>
                  <th
                    style={{
      ...
      {showEditModal && editingCase && (
        ...
        >
          ...
          >
            ...
            >
              <div
                style={{ fontWeight: 600, fontSize: "0.98rem", color: "#fff" }}
              >
                {isNewCase
                  ? "➕ Add New Evaluation Case"
                  : `✏️ Edit Case ${editingCase.case_id}`}
              </div>
              <button
                type="button"
            ...
            >
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditingCase({ ...editingCase, case_id: e.target.value })
                  }
                  style={{
                    width: "100%",
                    padding: "8px 12px",
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setEditingCase({ ...editingCase, question: e.target.value })
                  }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditingCase({
                      ...editingCase,
                      expected_section: e.target.value,
                    })
                  }
