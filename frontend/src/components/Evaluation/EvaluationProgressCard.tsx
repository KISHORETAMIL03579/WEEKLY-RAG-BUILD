...
export const EvaluationProgressCard: React.FC<EvaluationProgressCardProps> = ({
  ...
  const calculatedPct = total > 0 ? Math.min(100, Math.max(0, Math.round((current / total) * 100))) : 0;
  ...
  return (
    ...
    >
      ...
      >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
              ...
              <span
                style={{
                  ...
                  color: isComplete ? '#34d399' : isRunning ? '#60a5fa' : 'var(--text-muted)',
                  fontSize: '0.78rem',
                  fontWeight: 700,
                  padding: '2px 8px',
                  borderRadius: '6px',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
          {elapsedSeconds > 0 && (
            ...
            >
              ⏱ Elapsed: <strong style={{ color: '#fff' }}>{formatDuration(elapsedSeconds)}</strong>
            </div>
          )}
          {isRunning && estRemainingSeconds > 0 && (
            ...
            >
              ⏳ Est. Left: <strong>~{formatDuration(estRemainingSeconds)}</strong>
            </div>
          )}
          {isRunning && onCancel && (
            <button
      ...
      {signals.length > 0 && (
        ...
        >
          <span style={{ color: 'var(--text-muted)', fontWeight: 600 }}>Signals:</span>
          {signals.map((sig, idx) => (
            <div
              key={idx}
              style={{
            ...
            >
              <strong style={{ color: sig.color || '#93c5fd' }}>{sig.label}:</strong>
      ...
      {configurationText && (
        ...
        >
          <span>{isRunning ? '🔒 Configuration Frozen:' : '⚙️ Configuration:'}</span>
